...
⚠️ Важно: Проект использует ES Modules (import / export). Из-за политики безопасности браузеров (CORS) проект не будет работать при открытии файла **index.html** напрямую двойным кликом. Требуется локальный веб-сервер.

* Самый быстрый и стандартный способ запустить проект — использовать утилиту **npx**. Это не требует установки лишних зависимостей в проект.
